...
main_dir = "ucb_no_obstacles_latest/"
try:
    os.mkdir(main_dir)
...
plot_roadmap = False
obstacle_pot = 0.1
reward_weights = {'connectivity': 5.0, 'increemental': 1.0}
...
for i in range(0, 20):
    ...
    if plot_roadmap:
        plt_roadmap.plot(y_track_nc_x, y_track_nc_y, label='dmp')
        plt_roadmap.scatter(y_track_nc_x, y_track_nc_y)
        plt_roadmap.legend()
    ...
    rx, ry, rt, path_cost = PRM_planning(sx, sy, gx, gy, obstacles=obstacles, guiding_paths=[dmp_time_kdtree],
                                         ...
                                         uniform_min=uniform_min, uniform_max_t=uniform_max_t)
